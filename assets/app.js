...
(function(){
  ...
  const API_URL = API_BASE_URL; // Alias used in Gigs/Bundles section

  // --- HELPER ---
  // Simple HTML escaping to prevent XSS in gig listings
  ...
  function updateAuthUI() {
    ...
    const solverStatusEl = document.getElementById('solverUsage'); // For solver page
  ...
  // --- SOLVER LOGIC (AI) ---
  const getAnswerBtn = document.getElementById('solveTextBtn'); 
  ...
  if(getAnswerBtn){
    ...
    getAnswerBtn.addEventListener('click', async ()=>{
      ...
      try {
        ...
        if (response.ok) {
            ...
            if (usageEl) usageEl.textContent = `Usage: ${data.count}/${data.limit} (Resets daily)`;
    ...
    if (newGigBtn) newGigBtn.addEventListener('click', () => {
        if(gigModal) gigModal.classList.add('show'); 
    });
    ...
    if (saveGig) {
        saveGig.addEventListener('click', async () => {
            ...
            try {
                // NOTE: We are intentionally NOT protecting this route on the frontend yet.
                const res = await fetch(`${API_URL}/api/gigs`, {
                    method: 'POST',
                    headers: { 'Content-Type': 'application/json' },
                    body: JSON.stringify({ title, desc, price, contact })
                });
